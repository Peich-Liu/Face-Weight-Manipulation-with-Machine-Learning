function [TT_task_table,ET_task_table] = separator(file_path)%file version 0.1.0 updated 1005
...
for i=1:len
    ...
    TT_task_table(i).Duration=task(i).textdata(2:j-1,3);
    TT_task_table(i).Period=task(i).textdata(2:j-1,4);
    ...
    ET_task_table(i).Duration=task(i).textdata(j:r,3);
    ET_task_table(i).Period=task(i).textdata(j:r,4);
